...
with left:
    ...
    if selected_knowledge and delete_btn:
        try:
            chroma_db.delete_knowledge_base(
                selected_knowledge)
...
with right:
    new_option = st.text_input(
        "Create New knowledge", placeholder="Name must be 3-63 character and alphanumeric")
    ...
    with st.form("file-uploader", clear_on_submit=True):
        ...
        if submitted and selected_knowledge:
            with st.spinner():
                ...
                embeddings = chroma_db.create_embeddings(
                    chunks, selected_knowledge)
...
with left2:
    if selected_knowledge:
        try:
            ...
            if selected_file and st.button("🗑️", help="Delete files from knowledge base"):
